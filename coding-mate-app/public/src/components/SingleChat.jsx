...
function SingleChat({ fetchAgain, socket, setFetchAgain, selectedChat }) {
  ...
  const sendMessage = async (msg) => {
    ...
    try {
      ...
      if (newAttach) formData.append("attachment", newAttach);
      console.log("Hello from Single Chat: ", newAttach);
      if (msg.length > 0) {
      ...
      const { data } = await axios.put(`${sendMessageRoute}`, formData, config);
